...
const computeHash = (password, salt)=>{
	...
	return crypto.createHash('sha256', salt).update(password).digest('base64')
}
...
app.post('/user/auth', (req, res) => {
	...
	const newhash = computeHash(password, userob.salt);
	
	if(userob.passwordHash != newhash){
...
app.post('/user', (req, res) => {
	...
	const test = getUser(userid);
	
	if(!test){
		//make new user
		const salt = new Date().toString();
		const hash = computeHash(password, salt);
		const NuserObj = {userid: userid, name: name, salt: salt, passwordHash: hash};
		saveUser(NuserObj);
